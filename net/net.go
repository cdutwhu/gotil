...
// URLParamStr : paramstr does not have '?'
func URLParamStr(values url.Values) (ok bool, paramstr string) {
	for param, vList := range values {
		sec := ""
		switch len(vList) {
		case 0:
			sec = param
		case 1:
			sec = fSf("%v=%v", param, vList[0])
			sec = sTrimSuffix(sec, "=")
		default:
			sec = fSf("%v=%v", param, vList)
		}
		paramstr += sec
		paramstr += "&"
		ok = true
	}
	return ok, sTrimSuffix(paramstr, "&")
}
